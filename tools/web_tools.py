...
def _search_backend_fallback_chain() -> list[str]:
    """Parse ``web.search_backend_fallback_chain`` from config.yaml.

    Returns a list of backend names (stripped, lower-cased) to try after the
    currently active backend. If unset or empty, returns [].
    """
    raw = (_load_web_config().get("search_backend_fallback_chain") or "").strip()
    if not raw:
        return []
...
def _search_with_fallbacks(
    ...
    """Execute ``provider.search``; if the provider is dead or returns an error,
    try the configured fallback chain. Returns the first successful result, or
    the last attempted provider's error.

    A provider returning ``{"success": True, "data": {"web": []}}`` is a
    HEALTHY provider with no results — treated as success (the provider
    responded, there's just nothing for that query). Only ``success=False``
    or an uncaught exception triggers the fallback chain.

    The returned dict includes a ``provider`` key naming the backend that
    produced the answer.
    """
    ...
    for fallback in _search_backend_fallback_chain():
        if fallback in tried:
            continue
        fb_provider = _wsp_get_provider(fallback)
        if fb_provider is None or not fb_provider.supports_search():
            logger.debug(
                "Fallback search backend '%s' is not registered/available", fallback
            )
            continue
        tried.append(fallback)
        logger.info(
            "Web search fallback to %s: '%s' (limit: %d)", fallback, query, limit
        )
        result = fb_provider.search(query, limit)
        if result.get("success") is True:
            result["provider"] = fallback
            return result

    # All providers exhausted — preserve the last result.
    result["provider"] = backend
    return result
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            ...
            # Use fallback chain when the primary provider fails (#544/#467).
            # _search_with_fallbacks returns the first successful result, or
            # the last provider's error if all fail. It adds a "provider" key
            # naming the backend that produced the answer.
            response_data = _search_with_fallbacks(provider, query, limit, backend)
            # Normalize the response: strip the internal "provider" key before
            # serializing so the caller sees the standard schema.
            provider_used = response_data.pop("provider", backend)
