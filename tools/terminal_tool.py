...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        in {"true", "1", "yes"},
        "docker_network": os.getenv("TERMINAL_DOCKER_NETWORK", "true").lower()
        in {"true", "1", "yes"},
...
def _create_environment(
    ...
    if env_type == "local":
        ...
        # it. The shared "default" container and RL/benchmark override
        # sandboxes keep their existing lifecycle.
        session_scoped = (
    ...
    with task_lock:
        ...
        try:
            ...
            new_env = _create_environment(
                ...
                host_cwd=_resolve_task_host_cwd(config, effective_task_id),
            )
...
def is_persistent_env(task_id: str) -> bool:
    ...
    if env is None:
        return False
    # Session-scoped docker containers survive between turns (torn down at
    # session close / idle timeout), even when persistent_filesystem is off.
        ...
        if background:
            ...
            while retry_count <= max_retries:
                ...
                try:
                    ...
                    # Retry ANY transient execute() exception (upstream parity).
                    # ``classification`` is kept for result enrichment (streak,
                    # hint, category) but does NOT gate the retry — upstream's
                    # terminal tests rely on a generic backend error being retried.
            ...
            if returncode != 0 and not exit_note:
                _min_error = f"Command exited with code {returncode}."
                if not output.strip():
