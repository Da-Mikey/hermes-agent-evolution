...
def skill_manage(
    ...
    if result.get("success"):
        ...
        try:
            from tools.skill_usage import bump_patch, forget, mark_agent_created, record_created, set_source_run_id
            from tools.skill_provenance import is_background_review
            if action == "create":
                if is_background_review():
                    ...
                    # Upstream: persist creation provenance + emit the
                    # 'created' lifecycle fact. Must run BEFORE the fork's
                    # provenance fields below — record_created() clears the
                    # sidecar record and would wipe them otherwise. Also
                    # runs after the gates so a rejected skill never emits
                    # a created fact.
                    record_created(
                        name,
                        agent_created=True,
                    ...
                    # Foreground, user-directed create: record provenance
                    # only (upstream semantics) — no curator eligibility.
                    record_created(
                        name,
                        agent_created=False,
                        task_id=task_id,
...
SKILL_MANAGE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            # replace_all/category/file_path/file_content/rationale) — old transcripts
