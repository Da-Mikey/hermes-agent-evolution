...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "terminal, and toolset, and only its final summary returns to you. "
        "Provide 'goal' for one task or 'tasks' for a parallel batch (limits "
        "and nesting rules are in the parameter descriptions).\n\n"
        "Runs in the background: dispatch returns immediately with live "
        "transcript paths; the completed result re-enters the conversation on "
        "its own. Do NOT wait or poll; continue other work.\n\n"
        "LIVE ORCHESTRATION: while children run, this tool also controls "
        "them — action='list' (live children + ids), action='steer' (redirect "
        "without stopping), action='stop' (end early; partial result still "
        "returns). Steer when a transcript shows a child drifting.\n\n"
        ...
        "COST GATE — delegate only when the subtask clears BOTH: expected "
        "context divergence from yours, and payoff exceeding dispatch "
        "overhead; marginal lookups do NOT clear it. Decide from LAST-KNOWN "
        "STATE, not probe agents.\n"
        ...
        "via 'context', including output language, tone, or style (e.g. "
        '"respond in Chinese").\n'
        "- Child summaries are SELF-REPORTS, not verified facts; for external "
        "side effects require a verifiable handle and verify it yourself — "
        "fetch the URL, stat the file, read back the content — before "
        "reporting success.\n"
        "- Leaf children (default) cannot call delegate_task, clarify, memory, "
        "send_message, or cronjob; orchestrators regain only delegate_task.\n"
        "- Children inherit the parent model and fallback chain unless pinned "
        "via delegation.provider / delegation.model in config.yaml. Results "
        "return as an array, one entry per task."
    )
