...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "COST GATE — delegate only when the subtask clears BOTH: (1) it is "
        "self-contained enough that its context would diverge from yours "
        "(measured context-divergence: deep analysis, independent "
        "exploration), and (2) its payoff exceeds dispatch overhead (setup + "
        "summary round-trip per child). Marginal lookups and anything "
        "answerable from your current context do NOT clear the gate.\n"
        "Decide from LAST-KNOWN STATE: base the delegation decision on what is "
        "already in your context rather than spawning probe agents to re-check "
        "visible facts — a child that only verifies what you can already see "
        "is a measurement, not a subtask.\n"
