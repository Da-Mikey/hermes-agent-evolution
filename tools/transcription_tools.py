...
def _prepare_audio_for_transcription(
    ...
    try:
        ...
        if (
            not Path(converted_path).is_file()
            or Path(converted_path).stat().st_size == 0
        ):
        ...
        try:
            ...
            try:
                ...
                return {
                    "success": False,
                    "transcript": "",
                    "error": "Local STT model returned an invalid result",
                }
            ...
            if not _local_model:
                return {
                    "success": False,
                    "transcript": "",
                    "error": "Local STT model failed to load on CPU fallback",
                }
            transcribe_result = _local_model.transcribe(file_path, **transcribe_kwargs)
            try:
                segments, info = transcribe_result
                ...
                return {
                    "success": False,
                    "transcript": "",
                    "error": "Local STT model returned an invalid result on CPU fallback",
                }
            transcript = _join_confident_segments(segments, local_config)
...
def _convert_caf_to_wav(file_path: str) -> Optional[str]:
    ...
    if afconvert:
        try:
            subprocess.run(
                [afconvert, file_path, wav_path, "-d", "LEI16", "-f", "WAVE"],
                check=True,
                capture_output=True,
                text=True,
                timeout=300,
...
def _transcribe_groq(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "Transcribed %s via Groq API (%s, lang=%s, %d chars)",
                Path(file_path).name,
                model_name,
                language or "auto",
                len(transcript_text),
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="hermes-stt-") as work_dir:
                try:
                    ...
                    if not any(
                        k in message
                        for k in ("unsupported", "corrupted", "invalid file")
                    ):
                        raise
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if direct_api_key:
        creds = {
            ...
            "base_url": str(get_env_value("XAI_BASE_URL") or "https://api.x.ai/v1")
            .strip()
            .rstrip("/"),
        }
    else:
    ...
    def _resolve_base_url(resolved_creds: Dict[str, str]) -> str:
        ...
        if resolved_creds.get("provider") == "xai-oauth":
            return (
                str(resolved_creds.get("base_url") or XAI_STT_BASE_URL)
                .strip()
                .rstrip("/")
            )
        return (
...
def _transcribe_elevenlabs(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    language_code = (
        _resolve_stt_language("elevenlabs", stt_config, extra_keys=("language_code",))
        or ""
    )
...
def _transcribe_prepared_audio(
    ...
    if Path(file_path).suffix.lower() == ".caf" and provider not in (
        "local",
        "local_command",
    ):
