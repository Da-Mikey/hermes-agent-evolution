...
def transcribe_audio(file_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    # If _get_provider returned "none" (e.g. the explicitly-configured cloud
    # provider is missing credentials), try a last-resort local fallback so
    # voice messages aren't silently dropped when faster-whisper is actually
    # installed. (#122)
    if provider == "none":
        if _HAS_FASTER_WHISPER:
            logger.info(
