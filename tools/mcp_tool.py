...
class MCPServerTask:
    ...
    __slots__ = (
        ...
        "_reconnect_retries", "_last_park_error", "_stateless_enabled",
            ...
            try:
                ...
                if self._reconnect_retries > _MAX_RECONNECT_RETRIES:
                    ...
                    if error_str != self._last_park_error:
                        ...
                        # Upstream's wording: names the state transition and
                        # the unwrapped root cause, which is what the
                        # log-hygiene test asserts on.
                        logger.warning(
...
# ---------------------------------------------------------------------------
# Trust-tier gating (operator config ``trust: untrusted``)
# ---------------------------------------------------------------------------
# Write-capable MCP tools on untrusted servers must pass the existing
        ...
        async def _call():
            ...
            if mcp_field(result, "is_error", "isError", default=False):
                ...
                if (
                    ...
                    or "not found" in lower_err
                    or "validation" in lower_err
                    or "invalid" in lower_err
                ):
                    error_text = (
                        f"{error_text}\n\n"
                ...
                return tool_error(_sanitize_error(
                    _truncate_mcp_text_result(error_text)
                ))
