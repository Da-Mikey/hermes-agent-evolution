...
CODING_AGENT_GUIDANCE = (
    ...
    "- Before exploring an unfamiliar Python codebase, call `repo_map` FIRST to get "
    "a structured overview (functions/classes/methods with file:line and "
    "hot-symbol ranking). It is far cheaper than 10+ sequential "
    "`read_file`/`search_files` calls. After the overview, read only the "
    ...
    "`read_file` accepts a list of up to 10 paths in a single call — prefer one "
    "batch `read_file` over multiple sequential ones when you already know which "
    "files you need.\n"
