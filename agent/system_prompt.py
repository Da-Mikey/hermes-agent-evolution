...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if getattr(agent, "_environment_probe", True):
        try:
            ...
            # Appended to the same list upstream moved the probe line into, so
            # the hint keeps sitting directly beside the probe it qualifies.
            try:
                from tools.python_install import get_install_hint
                _install_hint = get_install_hint()
                if _install_hint:
                    post_workspace_parts.append(_install_hint)
            except Exception:
                pass
        except Exception:
