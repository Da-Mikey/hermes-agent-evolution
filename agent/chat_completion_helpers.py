...
def _prompt_cache_scope_for_agent(agent) -> "str | None":
    ...
    """
    explicit = str(getattr(agent, "cache_key", None) or "")
    if explicit:
        return explicit
    try:
    ...
    *,
    ...
    if reason in {
        ...
        if (not fallback_already_active) or (
            ...
            _cooldown = min(60.0 * (2 ** backoff_count), 14400.0)
            ...
            logger.info(
                ...
                backoff_count, int(_cooldown), _cooldown / 60, backoff_count + 1,
            )
    ...
    if should_skip_candidate(fb_ident, current_ident):
        logger.warning(
            ...
            fb_provider,
            fb_model,
            current_ident.base_url or current_ident.provider,
        )
        return agent._try_activate_fallback(reason=reason, api_error=api_error)
    ...
    try:
        ...
        if not fb_api_mode_explicit and fb_api_mode == "chat_completions":
            if fb_provider == "openai-codex":
                ...
                # pre-resolve hint check above never sees it; a provider named
                # "anthropic" needs the same treatment. Match the host
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _emit_stream_end(
        *, final_text: str, finished: bool, error: str | None
    ) -> None:
    ...
    if agent.api_mode == "codex_responses":
        ...
        try:
            ...
            _emit_stream_end(
                final_text=_stream_final_text(response), finished=True, error=None
            )
            return response
        except Exception as exc:
        ...
        def _bedrock_call():
            ...
            try:
                ...
                try:
                    ...
                    plugin_reasoning_observer = (
                        has_reasoning_stream_observer_hooks()
                    )
                except Exception:
                    logger.debug(
                        "plugin reasoning stream observer check failed",
                        exc_info=True,
                    )
                ...
                streamed_response = stream_converse_with_callbacks(
                    ...
                    or agent.stream_delta_callback
                    or plugin_reasoning_observer
                    else None,
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        for chunk in _iter_provider_stream_chunks(
            ...
            try:
                ...
                try:
                    _diag["bytes"] = int(_diag.get("bytes", 0)) + _estimate_chunk_bytes(
                        chunk
                    )
                except Exception:
                    pass
                ...
                try:
                    ...
                    if deltas_were_sent["yes"]:
                        _partial_tool_in_flight = (
                            bool(result.get("partial_tool_names"))
