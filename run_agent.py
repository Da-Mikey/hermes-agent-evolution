...
class AIAgent:
    ...
    # Default False: caller-supplied SessionDB is almost always a shared
    ...
    def _effective_lmstudio_context_length(
        ...
        runtime_value = getattr(
            runtime_context_length, "context_length", runtime_context_length
        )
        runtime = (
            runtime_value
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        if (self.provider or "").strip().lower() != "lmstudio":
            return None
        if (
            getattr(self, "lmstudio_load_mode", "explicit") or "explicit"
        ).strip().lower() == "jit":
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        # Fork semantics: report uses_implicit_default=False — the floor is a
        # deliberate per-model mitigation, not a fallback default, so the
        # local-endpoint short-circuit in _compute_non_stream_stale_timeout
        # does not disable stale detection for users running reasoning models
        # on a local NIM endpoint.
    ...
    def _execute_tool_calls(
        ...
        if hasattr(self, "_emit_plan_before_tool_calls"):
        ...
        try:
            ...
            if hasattr(self, "_check_step_divergence_after_tool_calls"):
