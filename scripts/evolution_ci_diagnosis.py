...
def create_child_issue(
    ...
    for check, failure in classified:
        body_lines.extend([
            f"### {check.name}",
            "",
...
def _cleanup_old_reports(max_days: int = 7) -> int:
    """Delete diagnosis reports older than max_days. Returns count deleted."""
    report_dir = _report_dir()
    if not report_dir.is_dir():
        return 0
    cutoff = datetime.now(timezone.utc) - timedelta(days=max_days)
    deleted = 0
    for entry in report_dir.glob("diagnosis-*.json"):
        try:
            # Parse date from filename: diagnosis-2026-08-09T181258Z.json
            stem = entry.stem  # diagnosis-2026-08-09T181258Z
            date_str = stem.replace("diagnosis-", "").replace("Z", "")
            ...
            continue
    if deleted:
        print(f"[ci-diagnosis] cleaned up {deleted} old report(s)")
    return deleted
...
def diagnose_prs(
    ...
    for pr in prs:
        ...
        for check in failed_checks:
            ...
            all_failures.append((check, failure))
            print(
