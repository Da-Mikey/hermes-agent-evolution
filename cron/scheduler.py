...
"""
        ...
        if _session_db:
            ...
            try:
                # Detect sessions that completed without producing any
                # tool calls — these are no-ops that should be visible
                # as failures, not hidden as successes. (#607)
                _tool_call_count = 0
                if agent is not None and hasattr(agent, "get_activity_summary"):
                    try:
                        _tool_call_count = agent.get_activity_summary().get(
                            "api_call_count", 0
                        )
                    except Exception:
                        pass
                _end_reason = "cron_noop" if _tool_call_count == 0 else "cron_complete"
                if _end_reason == "cron_noop":
                    logger.warning(
                        "Cron session %s ended with zero tool calls — marking as cron_noop",
                        _cron_session_id,
                    )
                _session_db.end_session(_cron_session_id, _end_reason)
            except (Exception, KeyboardInterrupt) as e:
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    """
    try:
        # Durable in-flight marker (issue 105): if the gateway dies while this
        # job runs, recover_interrupted_jobs() makes the loss visible (and
        ...
        def _process_job(job: dict) -> bool:
            ...
            try:
                ...
                if should_deliver:
                    ...
                    if delivery_error:
                        # Fallback: write the undeliverable content to a well-known file
                        # so the user can retrieve it when the primary channel is down.
                        evolution_dir = Path(
                            os.environ.get("HERMES_EVOLUTION_DIR")
                            or str(
                                Path.home()
                                / ".hermes"
                                / "profiles"
                        ...
                        fallback_path = evolution_dir / "last_delivery_fallback.md"
                        try:
                            fallback_path.write_text(
                                f"# Cron Delivery Fallback\n\n"
                                f"**Job:** {job.get('name', job['id'])}\n"
                                f"**Job ID:** {job['id']}\n"
                                f"**Error:** {delivery_error}\n"
                                f"**Timestamp:** {_hermes_now().isoformat()}\n\n"
                                f"---\n\n"
                                f"{deliver_content}\n"
                            )
                            logger.info(
                                "Job '%s': delivery fallback written to %s",
                                job["id"],
                                fallback_path,
                            )
                        except OSError as fbe:
                            logger.error(
                                "Job '%s': failed to write delivery fallback: %s",
                                job["id"],
                                fbe,
                            )
