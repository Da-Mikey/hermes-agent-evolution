...
def _patch_managed_uv(request):
    ...
    # **kwargs on all three: upstream keeps adding keyword-only options to these
    # helpers (repair_observer, most recently). A fixture that pins the old
    # signature turns every test in this file into a TypeError the moment
    # upstream widens one, which says nothing about the code under test.
    def _fake_resolve_uv(*_a, **_kw):
        return shutil.which("uv")

    def _fake_ensure_uv(*_a, **_kw):
        return shutil.which("uv")

    def _fake_update_managed_uv(*_a, **_kw):
...
class TestCmdUpdateBranchFallback:
    ...
    def test_diverged_fork_skips_upstream_main_automerge(self, mock_run, capsys):
        ...
        # Patch on update_cmd, not main: upstream extracted the update command
        # into hermes_cli/update_cmd.py, so _sync_with_upstream_if_needed now
        # resolves these helpers from THAT module's globals. main only
        # re-exports the function object, so patching main's globals no longer
        # intercepts anything. Still called through hm — that is the entry
        # point the CLI uses.
        from hermes_cli import update_cmd as uc

        with patch.object(uc, "_has_upstream_remote", return_value=True), patch.object(
            uc, "_count_commits_between", side_effect=[503, 995]
        ):
