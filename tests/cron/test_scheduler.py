...
class TestDeliveryFallbackFile:
    """Verify that delivery failures write a fallback file (issue #525)."""
    ...
    def test_fallback_written_on_delivery_error(self, tmp_path, caplog):
        """When _deliver_result returns an error, a fallback file must be written."""
        fallback_dir = tmp_path / "evolution"
        fallback_file = fallback_dir / "last_delivery_fallback.md"

        with (
            patch("cron.scheduler.get_due_jobs", return_value=[self._make_job()]),
            patch(
                "cron.scheduler.run_job",
                return_value=(True, "# output", "Test content", None),
            ),
            patch(
                "cron.scheduler.save_job_output", return_value=str(tmp_path / "out.md")
            ),
            patch(
                "cron.scheduler._deliver_result",
                return_value="telegram: connection refused",
            ),
            patch("cron.scheduler.mark_job_run"),
            patch.dict(os.environ, {"HERMES_EVOLUTION_DIR": str(fallback_dir)}),
        ):
            from cron.scheduler import tick
        ...
        assert fallback_file.exists(), "Fallback file should be created"
        content = fallback_file.read_text()
        assert "Test content" in content
        assert "connection refused" in content
        assert "fallback-test-job" in content

    def test_no_fallback_on_successful_delivery(self, tmp_path):
        """When delivery succeeds, no fallback file should be written."""
        fallback_dir = tmp_path / "evolution"
        fallback_file = fallback_dir / "last_delivery_fallback.md"
        ...
        assert not fallback_file.exists(), (
            "Fallback file should NOT be created on success"
        )

    def test_fallback_written_on_exception(self, tmp_path, caplog):
        """When _deliver_result raises, the fallback file should still be written."""
        fallback_dir = tmp_path / "evolution"
        fallback_file = fallback_dir / "last_delivery_fallback.md"

        def _raise_error(*args, **kwargs):
            raise RuntimeError("timeout sending to Telegram")

        with (
            patch("cron.scheduler.get_due_jobs", return_value=[self._make_job()]),
            patch(
                "cron.scheduler.run_job",
                return_value=(True, "# output", "Urgent alert", None),
            ),
            patch(
                "cron.scheduler.save_job_output", return_value=str(tmp_path / "out.md")
            ),
            patch("cron.scheduler._deliver_result", side_effect=_raise_error),
            patch("cron.scheduler.mark_job_run"),
            patch.dict(os.environ, {"HERMES_EVOLUTION_DIR": str(fallback_dir)}),
        ):
            from cron.scheduler import tick
        ...
        assert fallback_file.exists(), "Fallback file should be created on exception"
        content = fallback_file.read_text()
        assert "Urgent alert" in content
        assert "timeout sending" in content
