...
class TestDDGSProviderSearch:
    ...
    def test_no_results_exception_return_provider_dead_error(self, monkeypatch):
        ...
        assert "alternative" in result["error"].lower()
    ...
    def test_hung_process_search_times_out_and_returns_failure(self, monkeypatch):
    ...
    def test_fast_inprocess_search_not_affected_by_timeout_wrapper(self, monkeypatch):
        """Happy-path guard: the timeout wrapper must not break a normal,
        fast search — results flow through unchanged."""
        _install_fake_ddgs(
            monkeypatch,
