...
class TestOrchestratorRoleBehavior(unittest.TestCase):
    ...
    @patch("tools.delegate_tool._load_config",
           return_value={"max_spawn_depth": 2})
    def test_intersection_preserves_agent_team_for_teammates(
        self, mock_cfg, mock_creds
    ):
        """A teammate child keeps the 'agent_team' toolset even though the
        parent (lead) doesn't have it enabled. The toolset is injected by
        _ensure_team_toolset (never by the model), and the parent
        intersection in _build_child_agent would otherwise silently drop
        it — leaving the teammate without task-list/peer-messaging tools.
        Capability is granted by team membership (check_fn-gated), not
        inherited, mirroring the orchestrator 'delegation' re-add above.
        """
        mock_creds.return_value = {
            "provider": None, "base_url": None,
            "api_key": None, "api_mode": None, "model": None,
        ...
        parent = _make_mock_parent(depth=0)
        parent.enabled_toolsets = ["terminal", "file"]  # no agent_team
        with patch("run_agent.AIAgent") as MockAgent:
            mock_child = _make_role_mock_child()
            MockAgent.return_value = mock_child
            delegate_task(
